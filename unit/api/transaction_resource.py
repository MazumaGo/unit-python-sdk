...
class TransactionResource(BaseResource):
    ...
    def get(
        self, transaction_id: str, account_id: str, include: Optional[str] = ""
        ...
        if response.status_code == 200:
            ...
            return UnitResponse[TransactionDTO](
                DtoDecoder.decode(data), DtoDecoder.decode(included)
            )
        else:
            return UnitError.from_json_api(response.json())

    def list(
        self, params: ListTransactionParams = None
    ) -> Union[UnitResponse[List[TransactionDTO]], UnitError]:
    ...
    def update(
        self, request: PatchTransactionRequest
    ) -> Union[UnitResponse[TransactionDTO], UnitError]:
        payload = request.to_json_api()
        response = super().patch(
            f"accounts/{request.account_id}/{self.resource}/{request.transaction_id}",
            payload,
        )
        if super().is_20x(response.status_code):
            data = response.json().get("data")
    ...
    def get_by_id_and_account(
        ...
        response = super().get(
            f"accounts/{account_id}/{self.resource}/{transaction_id}",
            {"include": include},
        )
        if super().is_20x(response.status_code):
            data = response.json().get("data")
