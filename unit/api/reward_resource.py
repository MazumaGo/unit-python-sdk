...
class RewardResource(BaseResource):
    ...
    def create(
        self, request: CreateRewardRequest
    ) -> Union[UnitResponse[RewardDTO], UnitError]:
    ...
    def get(
        self, reward_id: str, include: Optional[str] = ""
    ) -> Union[UnitResponse[RewardDTO], UnitError]:
        response = super().get(f"{self.resource}/{reward_id}", {"include": include})
        if super().is_20x(response.status_code):
            data = response.json().get("data")
            included = response.json().get("included")
            return UnitResponse[RewardDTO](
    ...
    def list(
        self, params: ListRewardsParams = None
    ) -> Union[UnitResponse[List[RewardDTO]], UnitError]:
        params = params or ListRewardsParams()
        response = super().get(self.resource, params.to_dict())
        if super().is_20x(response.status_code):
            data = response.json().get("data")
            included = response.json().get("included")
            return UnitResponse[List[RewardDTO]](
                DtoDecoder.decode(data), DtoDecoder.decode(included)
            )
        else:
            return UnitError.from_json_api(response.json())
